...
void StepHandler(String data){
    String sdata[4];
    split(sdata, 4, data, '%');
    int dirpin = sdata[1].toInt();
    int stpin = sdata[2].toInt();
    int steps = sdata[3].toInt();
    int x;
    if(sdata[0]=="F"){  // set direction
...
void SerialParser(void) {
  ...
  if (cmd == "dw") {
      ...
      StepHandler(data); // data = direction(F or B),dir pin,step pin,#steps  
  }  
  else if (cmd == "aw") {
