...
class DAQ(object):
    ...
    def SetRepitition(self, n):
        ...
        self.df_pmt_columns = [ "pmt%05d"%(i+1) for i in range(n) ]
        self.df_det_columns = [ "det%05d"%(i+1) for i in range(n) ]
    ...
    def RunDAQ(self, ID, n_rep = 5, save_dir="./data", prefix="fiber", suffix=None, positions=None):
        ...
        while( rep_count <= self.run_reptition):
            ...
            pmt_name = "pmt%05d"%rep_count
            det_name = "det%05d"%rep_count
            ...
            for i in range(len(run_range)):
                ...
                self.curr_dataframe[det_name][i] = analog[0]*0.0001
                self.curr_dataframe["pos"][i] = self.board_pos/200          # dividing by 2 to make up for half speed multiplier
